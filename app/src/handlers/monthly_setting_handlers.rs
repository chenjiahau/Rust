use actix_web::{get, post, web, HttpRequest, Responder};
use httpstatus::StatusCode;
use sea_orm::{ActiveModelTrait, ColumnTrait, Condition, EntityTrait, QueryFilter, QueryOrder, Set};
use serde::Deserialize;
use uuid::Uuid;

use crate::models::{monthly_setting_models};
use crate::utils::app_state::AppState;
use crate::utils::api_response::{get_user_id_from_request, response};
use crate::utils::message;

#[derive(Debug, Deserialize)]
struct YearMonthParam {
    year: i32,
    month: i32,
}

#[utoipa::path(
    get,
    path = "/api/monthly_setting/count/{year}/{month}",
    params(
        ("year" = i32, Path, description = "Year of the monthly budget"),
        ("month" = i32, Path, description = "Month of the monthly budget")
    ),
    security(("bearerAuth" = [])),
    tag = "Monthly Setting",
)]
#[get("/count/{year}/{month}")]
async fn get_monthly_setting_count(
    req: HttpRequest,
    app_state: web::Data<AppState>,
    param: web::Path<YearMonthParam>,
) -> impl Responder {
    let user_id = get_user_id_from_request(&req).unwrap();

    // Fetch monthly settings for the user
    let result = entity::monthly_settings::Entity::find()
        .filter(
            Condition::all()
                .add(entity::monthly_settings::Column::UserId.eq(Uuid::parse_str(user_id.as_str()).unwrap()))
                .add(entity::monthly_settings::Column::Year.eq(param.year))
                .add(entity::monthly_settings::Column::Month.eq(param.month)),
        )
        .order_by(entity::monthly_settings::Column::CreatedAt, sea_orm::Order::Asc)
        .all(&app_state.db)
        .await;

    // Return the count of monthly settings
    let count = result.unwrap().len();
    let res = monthly_setting_models::MonthlySettingCountResponseModel { count };
    let success_message = message::SuccessMessage::Success;
    response(
        StatusCode::Ok,
        success_message.to_code(),
        Some(success_message.to_string()),
        Some(res),
    )
}

#[utoipa::path(
    get,
    path = "/api/monthly_setting/{year}/{month}",
    params(
        ("year" = i32, Path, description = "Year of the monthly setting"),
        ("month" = i32, Path, description = "Month of the monthly setting")
    ),
    security(("bearerAuth" = [])),
    tag = "Monthly Setting",
)]
#[get("/{year}/{month}")]
async fn get_monthly_setting(
    req: HttpRequest,
    app_state: web::Data<AppState>,
    param: web::Path<YearMonthParam>,
) -> impl Responder {
    let user_id = get_user_id_from_request(&req).unwrap();

    // Fetch monthly budgets for the user
    let option_model = entity::monthly_settings::Entity::find()
        .filter(
            Condition::all()
                .add(entity::monthly_settings::Column::UserId.eq(Uuid::parse_str(user_id.as_str()).unwrap()))
                .add(entity::monthly_settings::Column::Year.eq(param.year))
                .add(entity::monthly_settings::Column::Month.eq(param.month)),
        )
        .one(&app_state.db)
        .await
        .unwrap();

    if option_model.is_none() {
        let error_message = message::ErrorMessage::MonthlySettingNotFound;
        return response(
            StatusCode::NotFound,
            error_message.to_code(),
            Some(error_message.to_string()),
            Option::<()>::None,
        );
    }

    // Return the monthly setting
    let model = option_model.unwrap();
    let res = monthly_setting_models::MonthlySettingResponseModel {
        id: model.id,
        user_id: model.user_id,
        month: model.month,
        year: model.year,
        income: model.income,
        deposit: model.deposit,
        created_at: model.created_at.to_string(),
        updated_at: model.updated_at.to_string(),
    };

    let success_message = message::SuccessMessage::Success;
    response(
        StatusCode::Ok,
        success_message.to_code(),
        Some(success_message.to_string()),
        Some(res),
    )
}

#[utoipa::path(
    post,
    path = "/api/monthly_setting/{year}/{month}",
    params(
        ("year" = i32, Path, description = "Year of the monthly setting"),
        ("month" = i32, Path, description = "Month of the monthly setting")
    ),
    security(("bearerAuth" = [])),
    tag = "Monthly Setting",
)]
#[post("/{year}/{month}")]
async fn create_monthly_setting(
    req: HttpRequest,
    app_state: web::Data<AppState>,
    param: web::Path<YearMonthParam>,
) -> impl Responder {
    let user_id = get_user_id_from_request(&req).unwrap();

    // Delete the existing monthly budgets for the user in the specified month and year
    let result = entity::monthly_settings::Entity::delete_many()
        .filter(
            Condition::all()
                .add(entity::monthly_settings::Column::UserId.eq(Uuid::parse_str(user_id.as_str()).unwrap()))
                .add(entity::monthly_settings::Column::Year.eq(param.year))
                .add(entity::monthly_settings::Column::Month.eq(param.month)),
        )
        .exec(&app_state.db)
        .await;

    if result.is_err() {
        let error_message = message::ErrorMessage::MonthlySettingNotDeleted;
        return response(
            StatusCode::InternalServerError,
            error_message.to_code(),
            Some(format!("{}: {}", error_message.to_string(), result.err().unwrap())),
            Option::<()>::None,
        );
    }

    // Fetch setting by user_id
    // Check if the setting exist
    let option_model = entity::settings::Entity::find()
        .filter(
            Condition::all()
                .add(entity::settings::Column::UserId.eq(Uuid::parse_str(user_id.as_str()).unwrap()))
        )
        .one(&app_state.db)
        .await
        .unwrap();

    if option_model.is_none() {
        let error_message = message::ErrorMessage::SettingNotFound;
        return response(
            StatusCode::NotFound,
            error_message.to_code(),
            Some(error_message.to_string()),
            Option::<()>::None,
        );
    }

    // Create monthly setting
    let model = option_model.unwrap();
    let active_model = entity::monthly_settings::ActiveModel {
        user_id: Set(Uuid::parse_str(user_id.as_str()).unwrap()),
        month: Set(param.month),
        year: Set(param.year),
        income: Set(model.default_income),
        deposit: Set(model.target_deposit as f64),
        ..Default::default()
    };

    let result = active_model.insert(&app_state.db).await;

    if result.is_err() {
        let error_message = message::ErrorMessage::MonthlySettingNotCreated;
        return response(
            StatusCode::InternalServerError,
            error_message.to_code(),
            Some(error_message.to_string()),
            Option::<()>::None,
        );
    }

    // Return the monthly setting
    let model = result.unwrap();
    let res = monthly_setting_models::MonthlySettingModel {
        id: model.id,
        user_id: Uuid::parse_str(user_id.as_str()).unwrap(),
        month: model.month,
        year: model.year,
        income: model.income,
        deposit: model.deposit,
        created_at: model.created_at.to_string(),
        updated_at: model.updated_at.to_string(),
    };

    let success_message = message::SuccessMessage::CreatedSuccessfully;
    response(
        StatusCode::Created,
        success_message.to_code(),
        Some(success_message.to_string()),
        Some(res),
    )
}